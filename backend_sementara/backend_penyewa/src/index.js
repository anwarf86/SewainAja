...
app.get("/api/readPenyewa/:id_penyewa", (req, res) => {
  const P_nama = req.params.user_email;
...
app.post("/api/createPenyewa", (req, res) => {
  ...
  const sqlQuery = "INSERT INTO penyewa (nama,email,password,foto_sim,no_wa,status) VALUE (?, ?, ?, ?, ?, ?)";
  db.query(sqlQuery, [P_nama, P_email,P_password,P_foto_sim,P_no_wa,P_status], (err, result) => {
    if (err) {
      console.log(err);
    } else {
      res.send(result);
...
app.put("/api/updatePenyewa", (req, res) => {
  ...
  db.query(sqlQuery, [ P_nama, P_password, P_foto_sim, P_no_wa, P_status, P_email], (err, result) => {
    if (err) {
      console.log(err);
    } else {
      res.send(result);
